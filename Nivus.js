...
const constrolsStyleBase =
  "#joystick,#kick{z-index:100;bottom:CONTROLS_MARGINvw}.neo{opacity:CONTROLS_OPACITY;background-color:#c2c2c255;box-shadow:6px 6px 10px 0 #a5abb133,-5px -5px 9px 0 #a5abb133;color:#dedede55;font-weight:bolder;font-size:1.5rem}.sizer{width:CONTROLS_WIDTH%;aspect-ratio: 1 / 1;}#joystick{left:CONTROLS_MARGIN%;overflow:visible}#thumb{width:40%;height:40%;background-color:#ecf0f3cc}#kick{right:CONTROLS_MARGIN%}button.neo:active{opacity:KICK_OPACITY}";
...
function createStoreButton() {
  ...
  store.innerHTML =
    '<svg xmlns="http://www.w3.org/2000/svg" xml:space="preserve" viewBox="0 0 407 407" fill="white" style="height:0.85em; width: auto"><path d="M402 84 323 5c-3-3-7-5-12-5H17C8 0 0 8 0 17v373c0 9 8 17 17 17h373c9 0 17-8 17-17V96c0-4-2-9-5-12zm-101 80H67V39h234v125z"></path><path d="M214 148h43c3 0 6-2 6-6V60c0-4-3-6-6-6h-43c-3 0-6 2-6 6v82c0 4 3 6 6 6z"></path></svg> Store';
...
function setupGameUI() {
  ...
  if (!getByDataHook("chat-toggle")) {
    ...
    button.innerHTML =
      '<svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 20 20"><path fill="white" d="M5.8 12.2V6H2C.9 6 0 6.9 0 8v6c0 1.1.9 2 2 2h1v3l3-3h5c1.1 0 2-.9 2-2v-1.82a.943.943 0 0 1-.2.021h-7zM18 1H9c-1.1 0-2 .9-2 2v8h7l3 3v-3h1c1.1 0 2-.899 2-2V3c0-1.1-.9-2-2-2"/></svg>';
...
function updateJoystickBounds() {
  ...
  joystickCenterX = joystickRect.left + joystickRect.width * 0.5;
  joystickCenterY = joystickRect.top + joystickRect.height * 0.5;
}
...
function updateJoystick(touch) {
  ...
  thumb.style.left =
    radius + distance * Math.cos(angle) - thumb.clientWidth * 0.5 + "px";

  thumb.style.top =
    radius + distance * Math.sin(angle) - thumb.clientHeight * 0.5 + "px";

  const directions = ["d", "sd", "s", "sa", "a", "wa", "w", "wd"];
  const direction =
    directions[
      Math.round((((angle + Math.PI * 2) % (Math.PI * 2)) * 4) / Math.PI) % 8
    ];
