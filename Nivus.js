document.head.appendChild(
  Object.assign(document.createElement("style"), {
    innerHTML:
      "#thumb,body{touch-action:none}body{user-select:none;height:100%}@media only screen and (max-device-width:480px){body{touch-action:manipulation}}.header,.rightbar{display:none!important}.round{border-radius:8px}.neo{box-shadow:6px 6px 10px 0 #a5abb133,-5px -5px 9px 0 #a5abb133;background:linear-gradient(145deg,#12181b,#101417);color:white}button.neo{padding:0.5rem 0.9rem;font-size:0.9rem}input[data-hook]{height:2rem}#joystick,#kick{position:fixed;right:1rem;bottom:5vh;width:80px;height:80px;border-radius:50%;display:flex;align-items:center;justify-content:center}#kick{right:calc(1rem + 90px)}#thumb{width:36px;height:36px;background:rgba(255,255,255,0.8);border-radius:50%;position:relative}#joystick[view=hidden],#kick[view=hidden]{display:none}",
  }),
);
document.querySelector(".gameframe").contentWindow.document.head.appendChild(
  Object.assign(document.createElement("style"), {
    innerHTML:
      ".room-view,.roomlist-view{height:100%;margin-top:0}.game-view>.top-section,.room-view{margin-top:0}.settings-view{width:100%;max-height:none}.game-view>[data-hook=popups]{background-color:#00000055}",
  }),
);

if (
  !localStorage.getItem("low_latency_canvas") ||
  localStorage.getItem("low_latency_canvas") == 1
) {
...
const constrolsStyleBase =
  "#joystick,#kick{z-index:100;bottom:CONTROLS_MARGINvw}.neo{opacity:CONTROLS_OPACITY;background-color:#c2c2c255;box-shadow:6px 6px 10px 0 #a5abb133,-5px -5px 9px 0 #a5abb133;color:#dedede55;font-family:Inter,system-ui,-apple-system,Segoe UI,Roboto,Ubuntu,Cantarell,Noto Sans,sans-serif}#joystick{left:2vw;width:CONTROLS_WIDTHvw;height:CONTROLS_WIDTHvw;display:flex;align-items:center;justify-content:center}#kick{right:3vw;width:CONTROLS_WIDTHvw;height:CONTROLS_WIDTHvw;border-radius:10px;display:flex;align-items:center;justify-content:center}#kick svg{height:50%}#joystick .sizer{width:100%;height:100%}#joystick .rounded{border-radius:50%}#thumb{transform:translate(-50%,-50%);position:absolute}#joystick[float]{position:fixed}#kick[float]{position:fixed}#joystick[view=hidden],#kick[view=hidden]{display:none}#thumb{left:50%;top:50%}#joystick{background:transparent}
";
...
function checkLoader() {
  if (
    !gameFrame.document.body.querySelector(".loader-view") &&
    gameFrame.document.body.querySelector(".choose-nickname-view")
  ) {
...
function init() {
  ...
  aboutHandler.style.cssText =
    "background: #1a2125; position: absolute; width: 100%; height: 100%; display: none; justify-content: center; flex-direction: column; align-items: center; margin: 0;";
  aboutHandler.innerHTML =
    '<div class="dialog basic-dialog" style="max-width: 50%;"><h1>About us</h1><p>We are Vixel Dev, a small development studio that wants the Haxball community to grow, without hurting its owners. This project is made with love for the community.</p><button data-hook="closeabout" style="position:absolute;top:10px;right:10px">Close</button></div>';
  ...
  body.parentNode
    .querySelector('[data-hook="closeabout"]')
...
function setupCopyright() {
  ...
  copyrightHandler.setAttribute(
    "style",
    "text-align:center;position:absolute;bottom:15px;width:100%; display: block",
  );
  copyrightHandler.innerHTML =
    "2024 Vixel Dev. Original game by Mario Carbajal (@basro)";
...
function updateUI() {
  ...
  if (body.querySelector(".roomlist-view")) {
    ...
    body.querySelector(".game-view") &&
    !body.querySelector(".room-link-view")
  ) {
    //Room admin
    copyright(false);
    showControls(false);
...
function createShareButton() {
  ...
  share.addEventListener("click", function () {
    console.log(
      "SHARE_MESSAGE🎮⚽️ Join my Haxball Mobile room by copying and pasting the following link: " +
        getByDataHook("link").value,
    );
  });
}
...
function createStoreButton() {
  ...
  store.innerHTML =
    '<svg xmlns="http://www.w3.org/2000/svg" xml:space="preserve" viewBox="0 0 407 407" fill="white" style="height:0.85em; width: auto"><path d="M402 84 323 5c-3-3-7-5-12-5H17C8 0 0 8 0 17v373c0 [...]';
...
function createSearchbar() {
  ...
  inputContainer.innerHTML =
    '<label>Search a room:</label><input data-hook="search" type="text">';
...
function createURLButton() {
  ...
  button.addEventListener("click", function () {
    if (!body.querySelector('[data-hook="input-url"]')) {
      ...
      urlForm.innerHTML =
        '<div class="label-input" style="background-color: transparent"><label>URL:</label><input data-hook="input-url" type="url"></div>';
...
function filterCountries(button) {
  const geoData =
    localStorage.getItem("geo_override") || localStorage.getItem("geo");
  ...
  if (geoData) {
    ...
    if (iconClass === "icon-cancel") {
      ...
      countryFilterHandler.innerHTML =
        "tr:not(:has(div.f-" + code + ")){display: none;}";
    }
...
function setupGameUI() {
  ...
  if (!getByDataHook("chat-toggle")) {
    ...
    button.innerHTML =
      '<svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 20 20"><path fill="white" d="M5.8 12.2V6H2C.9 6 0 6.9 0 8v6c0 1.1.9 2 2 2h1v3l3-3h5c1.1 0 2-.9 2-2v-1.82a.943.[...]';
    button.addEventListener("click", chatToggle);
    body.querySelector(".sound-button-container").parentNode.prepend(button);
  }

  if (firstTime) {
    const dr = body.querySelector(".drag");
    if (dr) dr.remove();
    ...
    const menu = getByDataHook("menu");
    if (menu) menu.innerHTML = '<i class="icon-menu"></i>';
    const inputStyleObj = chat && chat.querySelector(".input");
    if (inputStyleObj) {
      inputStyleObj.style.display = "none";
...
function prefabMessage(msg) {
  ...
  const input = chatbox && chatbox.querySelector("input");
  if (!input) return;
  input.focus();
  input.value = msg;
...
function chatToggle() {
  const chat = body.querySelector(".chatbox-view");
  const inputStyle = chat && chat.querySelector(".input");
  if (!inputStyle) return;

  inputStyle.style.display = inputStyle.style.display === "none" ? "block" : "none";
  if (inputStyle.style.display == "block") {
    const inp = chat.querySelector("input");
    if (inp) inp.focus();
  }
}
...
function onControlsSettingsInput() {
  ...
  if (inputs.length >= 3)
    updateControlsOptions(
      ...
      inputs[1].children[2].value,
      inputs[2].children[2].value,
    );
}

function updateControlsOptions(w, m, o, f = false) {
  if (!controlsHandler) return;
  if (f) {
    let inputs = inputOptionsHandler.querySelectorAll(".option-row");
    if (inputs.length >= 3) {
...
function updateJoystick(touch) {
  ...
  if (th) {
    th.style.left = thumbX - rect.left - th.clientWidth / 2 + "px";
    th.style.top = thumbY - rect.top - th.clientHeight / 2 + "px";
  }
...
function resetJoystick() {
  ...
  const th = document.getElementById("thumb");
  if (th) {
    th.style.left = joystick.clientWidth / 2 - th.clientWidth / 2 + "px";
    th.style.top = joystick.clientHeight / 2 - th.clientHeight / 2 + "px";
  }
  emulateKeys("");
}
...
function emulateKeys(str) {
  ...
  try {
    gameFrame.document.dispatchEvent(
      new KeyboardEvent(keys["w"], { code: "KeyW" }),
    );
    gameFrame.document.dispatchEvent(
      new KeyboardEvent(keys["a"], { code: "KeyA" }),
    );
    gameFrame.document.dispatchEvent(
      new KeyboardEvent(keys["s"], { code: "KeyS" }),
    );
    gameFrame.document.dispatchEvent(
      new KeyboardEvent(keys["d"], { code: "KeyD" }),
    );
  } catch {}
}
...
function setupControls() {
  ...
  inputOptionsHandler.innerHTML =
    '<div class="dialog settings-view" style="height:min-content"><h1>Controls</h1><button data-hook="closeinput" style="position:absolute;top:12px;right:10px">Back</button><div class="tabcontent"><div class="option-row"><div>Width</div><div class="value">20</div><input type="range" min="10" max="40" value="20"></div><div class="option-row"><div>Margin</div><div class="value">5</div><input type="range" min="0" max="20" value="5"></div><div class="option-row"><div>Opacity</div><div class="value">1</div><input type="range" min="0.2" max="1" step="0.1" value="1"></div><div style="display:flex;gap:8px;margin-top:10px"><button data-hook="resetinput">Reset</button></div></div></div>';
  body.parentNode.appendChild(inputOptionsHandler);
  body.parentNode
    .querySelector('[data-hook="closeinput"]')
    .addEventListener("click", function () {
      inputOptionsHandler.setAttribute("hidden", "");
      showControls(false);
    });
  body.parentNode
    .querySelector('[data-hook="resetinput"]')
    .addEventListener("click", function () {
      updateControlsOptions(20, 5, 1, true);
    });
  inputOptionsHandler
    .querySelectorAll(".option-row")[0]
    .children[2].addEventListener("input", onControlsSettingsInput);
  inputOptionsHandler
    .querySelectorAll(".option-row")[1]
    .children[2].addEventListener("input", onControlsSettingsInput);
  inputOptionsHandler
    .querySelectorAll(".option-row")[2]
    .children[2].addEventListener("input", onControlsSettingsInput);
  ...
  kickButton.innerHTML =
    '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M290 49c-16 0-32 14-38 36-6 25 5 48 22 52 18 5 39-10 45-35 7-25-5-48-22-52l-7-1zM89 68 78 87c32 16 63 34 96 47l28-12c-4[...]'</svg>';
  kickButton.addEventListener("touchstart", function () {
    kick("keydown");
  });
  kickButton.addEventListener("touchend", function () {
    kick("keyup");
  });
...
function getDigitalStickState(x, y) {
  ...
  if (Math.abs(x) < centerThreshold && Math.abs(y) < centerThreshold) {
    return {
      changed: previousDigitalStickState !== "Center",
      direction: "Center",
    };
  }
...
function getAnalogStickState(x, y) {
  ...
  if (Math.abs(x) < centerThreshold && Math.abs(y) < centerThreshold) {
    return {
      changed: previousAnalogStickState !== "Center",
      direction: "Center",
    };
  }
